    ...
    <Layout>
      <div className="space-y-10 animate-in fade-in duration-1000">
        ...
        <header className="space-y-4">
          ...
          <h1 className="text-[32px] font-medium tracking-display text-black dark:text-linear-text-primary">
            {t('welcome')} <br />
            <span className="text-linear-light-text-muted dark:text-linear-text-tertiary">{t('todayHappening')}</span>
          </h1>
        </header>
        ...
        <section className="space-y-4 pt-4">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-[14px] font-semibold text-linear-light-text-muted dark:text-linear-text-secondary uppercase tracking-wider">{t('recentDiscoveries')}</h2>
            <Link href="/board" className="text-[12px] text-linear-light-text-muted dark:text-linear-text-tertiary hover:text-linear-indigo flex items-center gap-1 transition-colors">
                ...
                <Card hoverable className="h-full p-5 flex flex-col justify-between border-linear-light-border dark:border-linear-border bg-white dark:bg-white/[0.01]">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-mono text-linear-light-text-muted dark:text-linear-text-tertiary">LIN-{post.id.slice(0, 3).toUpperCase()}</span>
                    </div>
                    <h3 className="text-[14px] font-medium text-black dark:text-linear-text-primary line-clamp-2 leading-snug">{post.title}</h3>
                  </div>
                  <div className="pt-4 flex items-center justify-between">
                    <span className="text-[12px] text-linear-light-text-muted dark:text-linear-text-tertiary">{post.author_name}</span>
                    <span className="text-[11px] text-linear-light-text-muted dark:text-linear-text-tertiary/50 uppercase tracking-tighter">
        ...
        <footer className="pt-10 border-t border-linear-light-border dark:border-white/[0.03] text-center">
          <p className="text-[11px] text-linear-light-text-muted dark:text-linear-text-tertiary/30 font-medium tracking-widest uppercase">
  ...
  return (
    <Card className="p-5 space-y-3 border-linear-light-border dark:border-linear-border bg-white dark:bg-linear-panel">
      ...
      <div className="space-y-1">
        <p className="text-[12px] font-medium text-linear-light-text-muted dark:text-linear-text-tertiary">{title}</p>
        <p className="text-[20px] font-semibold text-black dark:text-linear-text-primary tracking-tight">{value}</p>
      </div>
    </Card>
  );
};
